...
def create_adjacency_list(line_numbers: List[str],
    ...
    for edge in edges:
        ...
        if True:
            ...
            # Control Flow edges
            if edge_type == 'CDG' and (edge['endType'] == 'JUMP_TARGET' or edge['endType'] == 'CONTROL_STRUCTURE' or edge['endType'] == 'IDENTIFIER'):
...
def process_directory(root_dir, slice_dir):
    ...
    for sub_dir in sub_dirs:
        ...
        adjacency_list, global_variable = create_adjacency_list(
            line_numbers, node_id_to_ln, edges, macro_candidate, function_range)
        ...
        for function_name, slice_ln, parent_method_id in call_lines:
            ...
            sliced_lines = create_forward_slice(
                combined_graph, slice_ln, parent_method_id, function_range, global_variable)
