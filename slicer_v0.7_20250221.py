...
# label을 추가해야된다면, function_call이 되었을때, 부모 메소드의 이름 또한 가지고 있어야 된다.
    ...
    for node_idx, node in enumerate(nodes):
        ...
        if ntype == 'CALL':
            ...
            if function_name.strip() in L_FUNCS:
                ...
                parent_method_name = get_parent_method_name(parent_method_id, nodes)
                function_calls.add((function_name, line_no, parent_method_id, parent_method_name))
    return function_calls

# 수집된 취약 함수가 어느 메소드에 속해있는지 그 메소드의 이름을 반환한다.
def get_parent_method_name(parent_method_id, nodes):
    for _, node in enumerate(nodes):
        if node.get('key').strip() == parent_method_id:
...
def calc_label(parent_method_name):
    label = 0
    try : 
        if parent_method_name.find("main") >=0:
            label = -1
            return label
...
def process_directory(root_dir, slice_dir):
    for sub_dir in os.listdir(root_dir):
        ...
        sink_flag = False
        # 함수 호출 부가 source가 되지 않나?
        source_flag = False

        # 수집된 취약함수 호출 라인하나당 스니펫을 생성한다.
        for function_name, slice_ln, parent_method_id, parent_method_name in call_lines:
